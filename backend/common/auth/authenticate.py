...
class JWTAuthentication(TokenAuthentication):
    def authenticate(self, request):
        ...
        if token_data.get('type') == 'application':
            ...
            if not application_token.get('is_active'):
                raise AppAuthenticationFailed()
            if application_token.get('white_active'):
                if request.get('ip') not in application_token.get('white_list'):
                    raise AppAuthenticationFailed()
            if application_token.get('access_num') <= 0:
                raise AppAuthenticationFailed()
            return application_token_dict,application_token
        else:
